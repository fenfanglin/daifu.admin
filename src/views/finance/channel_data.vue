...
export default {
	...
	methods: {
		getList() {
			...
			that.loading = true
			that.request({
				url: 'finance/channel_data'
			}).then(res => {
				that.orderBarCharData = {head:'今日通道订单数',title:res.data.title_order,yesterday_order:res.data.yesterday_order,today_order:res.data.today_order,today_rate:res.data.today_rate,yesterday_rate:res.data.yesterday_rate}
