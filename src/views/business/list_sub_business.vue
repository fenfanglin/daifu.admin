<template>
	<div class="app-container">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['business_sub:save:add']">新增</el-button>
			</el-col>
			<el-col :span="1.5">
				<el-button type="danger" icon="el-icon-delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['business_sub:delete']">删除</el-button>
			</el-col>
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-download" @click="handleExport" v-hasPermi="['business_sub:export']">导出</el-button>
			</el-col>
		</el-row>
      ...
      <el-table-column label="订单费率" prop="order_rate" width="80" />
			...
			<el-table-column label="所属四方" prop="parent_name" :show-overflow-tooltip="true" width="120" />
			...
			<el-table-column label="可提现金额" width="150">
				<template slot-scope="scope">
					<span v-if="scope.row.allow_withdraw <= 0" class="text-danger">{{scope.row.allow_withdraw}}</span>
					<span v-else class="text-success">{{scope.row.allow_withdraw}}</span>
				</template>
			</el-table-column>
			<el-table-column label="注册时间" prop="create_time" width="220" />
			<el-table-column label="操作" width="400">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="edit(scope.row)" v-hasPermi="['business_sub:save:edit']" plain>编辑</el-button>
					<el-button type="primary" class="tb-button" icon="el-icon-s-finance" @click="editRecharge(scope.row)" v-hasPermi="['business_sub:save_recharge']" plain>充值</el-button>
					<!-- <el-button type="primary" class="tb-button" icon="el-icon-user-solid" @click="login(scope.row)" v-hasPermi="['business_sub:login']" plain>登录</el-button> -->
					<el-button type="primary" class="tb-button" icon="el-icon-lock" @click="editPassword(scope.row)" v-hasPermi="['business_sub:change_password']" plain>密码</el-button>
					<el-button type="success" class="tb-button" icon="el-icon-success
" @click="enable(scope.row)" v-hasPermi="['business_sub:enable']" plain>启用</el-button>
					<el-button type="danger" class="tb-button" icon="el-icon-error" @click="disable(scope.row)" v-hasPermi="['business_sub:disable']" plain>禁用</el-button>
					<!-- <el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['business_sub:delete']" plain>删除</el-button> -->
				</template>
			</el-table-column>
		</el-table>
...
<script>
export default {
	name: "business_list_sub_business", // 路由上 '/' 换成 '_'
	data() {
		return {
			user: this.getUser(),
	...
	methods: {
		...
		submitFormPassword: function() {
			...
			that.$refs["form"].validate(valid => {
				if (valid) {
					that.request({
						url: "business_sub/change_password",
						data: that.form,
					}).then(res => {
						that.cancel();
						that.getList();
		...
		submitFormRecharge: function() {
			...
			that.$refs["form"].validate(valid => {
				if (valid) {
					...
					that.request({
						url: "business_sub/save_recharge",
						data: form,
					}).then(res => {
						that.cancel();
						that.loading = true;
		...
		login(row) {
			...
			that.request({
				url: "business_sub/login",
				data: {
					no: row.no
				},
			}).then(res => {
				let href = process.env.VUE_APP_BUSINESS_HOST + res.data.params
		...
		handleExport() {
			...
			that.exportExcel("business_sub/export", this.queryParams, "管理员").then(res => {
